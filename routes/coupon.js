...
router.post('/claim', (req, res) => {
  ...
  if (!userId || !couponId) return error(req, res, '缺少参数')
...
router.post('/apply', (req, res) => {
  ...
  const available = userCoupons.map(uc => {
    ...
    if (coupon.type === 'reduction') {
      ...
      if (subtotal >= coupon.threshold) discount = coupon.value
    } else if (coupon.type === 'discount') {
      // 折扣券：满 threshold 打 value 折
      if (subtotal >= coupon.threshold) discount = Math.round(subtotal * (1 - coupon.value))
    }
    return {
      userCouponId: uc.id,
      couponId: coupon.id,
